...
create table if not exists concepts       ( id text primary key, data jsonb not null, updated_at timestamptz default now() );
create table if not exists stages         ( id text primary key, data jsonb not null, updated_at timestamptz default now() );
...
do $$ declare t text;
begin
  foreach t in array array['team','accounts','profiles','products','concepts','stages','settings','tasks','scripts','script_entries','daily_entries','daily_hooks'] loop
...
-- ---------- live sync ----------
alter publication supabase_realtime add table team, accounts, profiles, products, concepts, stages, settings, tasks, scripts, script_entries, daily_entries, daily_hooks;
